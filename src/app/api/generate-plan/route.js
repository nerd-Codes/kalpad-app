    ...
    switch (mode) {
        ...
        default:
            ...
            **STRATEGY DATA:**
            ...
            - Coverage: ${triageData.estimated_coverage}%.
            - Focus: ${JSON.stringify(triageData.emphasized_topics.map(t => t.topic))}
            - Skip: ${JSON.stringify(triageData.skipped_topics.map(t => t.topic))}

            **YOUR TONE & TASKS:**
            1.  **Direct Acknowledgment:** Start by addressing their specific request. "You asked for '${userRequest}', so here is the deal..."
            2.  **The 'Why' (The Trade-off):** Explain the strategy clearly. "We are skipping [Skipped Topic] because it's a time-sink. We are doubling down on [Focus Topic] because that's where the marks are."
            3.  **The Reality Check:** If they asked for too much in too little time, call it out. "Your goal of 100% coverage is impossible in 5 days, so I've optimized for a safe 80% score instead."
            4.  **No Fluff:** Do not say "We will build a strong foundation." Say "We will master Calculus first because Physics depends on it."

            **UNBREAKABLE RULE:** Output ONLY JSON: { "overall_approach": "..." }
            `;
            break;
    }
...
export async function POST(request) {
  ...
  // --- GUEST CONSTRAINT CHECK ---
  const todayCheck = new Date();
  const examDateObj = new Date(examDate);
  const diffTime = Math.abs(examDateObj - todayCheck);
  ...
  const stream = new ReadableStream({
    ...
    if (useDocuments && !isGuest) {
        ...
        **CRITICAL JSON SCHEMA (Return ONLY this object and nothing else):**
        ...
        userRequest = triageData.user_request || " ";
                ...
                **  MODE-SPECIFIC PLANNING DIRECTIVE (UNBREAKABLE):**
                ${plannerDirectives}
                  ...
                  **CRITICAL JSON SCHEMA:** Return a JSON array of exactly 4 objects. Each object represents a week and MUST have these keys:
                  ...
                  - "hidden_dependencies": (array of strings) List specific prerequisites required for this week's topics (e.g. "Requires basic integration").
                  - "trap_topics": (array of strings) List any topics that may appear simple but are time-consuming.
                `;
        ...
        for (const weekData of comprehensiveWeeklyPlan) {
            ...
            const weeklyBatchPrompt = `
              ...
              **RED TEAM REPORT (CRITICAL DEPENDENCIES):**
              The Architect identified these hidden requirements for this week: 
              ${JSON.stringify(weekData.hidden_dependencies || [])}
              *Instruction: Ensure these prerequisites are scheduled EARLY in the week.*

                **SILENT ALARM REPORT (TIME TRAPS):**
                The Architect identified these potential time sinks for this week:
                ${JSON.stringify(weekData.trap_topics || [])}
                *Instruction: If these topics are present, allocate EXTRA time and a higher difficulty rating for these topics to ensure thorough coverage without overruns.*
              ...
              **GOLDEN QUESTION PROTOCOL (MANDATORY):**
              For every "Hard" or "Intense" topic, or any "Emphasized" topic:
              1. You MUST generate a "Golden Question" (The single most important exam-style question for that topic).
              2. Add this question as a specific **sub_topic** with type "Challenge".
                ...
                "weekly_plan": [
                  {
                    ...
                    "sub_topics": [
                      ...
                      {
                        "text": "GOLDEN QUESTION: [Insert Question Here]",
                        "completed": false,
                        "difficulty": "Hard",
                        "type": "Challenge"
                      }
